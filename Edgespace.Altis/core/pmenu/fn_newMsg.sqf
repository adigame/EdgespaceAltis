...
switch(_type) do
{
	...
	case 1:
	{
		if(isNUll life_smartphoneTarget) exitWith {hint format["No Recipient Selected!"];};
	...
	case 2:
	{
		if(({side _x == west} count playableUnits) == 0) exitWith {hint format["There are currently no police online."];};
