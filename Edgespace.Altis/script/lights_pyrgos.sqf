if (life_kw_pyrgos) exitWith {hint "You have already initiated load shedding in Pyrgos you will need to wait 10 minutes"};
...
{
	life_kw_pyrgos = true;
	hint "You have initiated initiated load shedding in Pyrgos";
	[[getPlayerUID player,profileName,"42"],"life_fnc_wantedAdd",false,false] spawn life_fnc_MP;
	[[0,format["!!! %1 from ESKOM has started load shedding in Pyrgos !!!", name player]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
	sleep 600;
	life_kw_pyrgos = false;
	[[0,format["!!! Load shedding has stopped Pyrgos !!!", name player]],"life_fnc_broadcast",west,false] spawn life_fnc_MP;
	hint "!!! load shedding has stopped Pyrgos !!!";
};